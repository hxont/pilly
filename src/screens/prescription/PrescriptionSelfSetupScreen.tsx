...
const PrescriptionSetupScreen = () => {
  ...
  const addMedicineManually = () => {
    ...
    if (trimmed !== "" && !medicineList.includes(trimmed)) {
  ...
  const submitPrescription = async () => {
    ...
    const requestData = {
      ...
      medicineNames: Array.from(new Set(medicineList.map((name) => name.trim()))), // 중복 제거 및 trim
    };

    console.log("📦 전송 데이터:", requestData); // 디버깅
        ...
        onPress={() =>
          navigation.navigate("PrescriptionSearchScreen", {
            onSelect: (selected: string) => {
              ...
              if (trimmed && !medicineList.some((m) => m.trim() === trimmed)) {
                setMedicineList((prev) => [...prev, trimmed]);
              }
            },
          })
